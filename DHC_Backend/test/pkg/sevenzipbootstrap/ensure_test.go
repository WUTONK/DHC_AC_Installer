...
func TestBuildTestURL(t *testing.T) {
	version := "25.01"
	// 使用内部测试函数来测试私有函数
	if result := sevenzipbootstrap.TestBuildTestURL(version); result != "https://www.7-zip.org/a/7z2501-mac.tar.xz" {
	...
	for _, tc := range testCases {
		result := sevenzipbootstrap.TestBuildTestURL(tc.version)
...
func TestExtractVersion(t *testing.T) {
	...
	for _, tc := range testCases {
		result := sevenzipbootstrap.TestExtractVersion(tc.input)
		if result != tc.expected {
...
func TestCompareVersion(t *testing.T) {
	...
	for _, tc := range testCases {
		result := sevenzipbootstrap.TestCompareVersion(tc.v1, tc.v2)
		if result != tc.expected {
