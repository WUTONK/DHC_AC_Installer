...
```json
{
    ...
    "rules": [
        { "pattern": "*.txt", "action": "backup", "backup": true },
        { "pattern": "*.acd", "action": "rename", "newName": ".cfg", "backup": false },
        { "pattern": "/sound", "action": "overwrite", "backup": true }
    ],
...
| `target` | string | 目标路径重映射（可将文件安装到与源路径不同的目标路径） |
...
| `dir/**` | **递归匹配** `dir` 下的所有文件和子目录 | `cars/**` 匹配 `cars` 下的一切 |
| `path/to/file` | 精确匹配（使用 `filepath.Match` glob 语法） | `cars/r34/data.acd` 只匹配该路径 |
| `/dirname` | 匹配名为 `dirname` 的目录路径 | `/car` 匹配路径中的 `car` 目录 |
...
```json
{
    ...
    "defaultAction": { "action": "overwrite", "backup": false },
    "rules": [
        { "pattern": "*.acd", "action": "rename", "newName": "data.acd", "backup": false },
        { "pattern": "*.txt", "action": "skip" }
    ],
    "overwriteStartingDir": "content"
}
```
